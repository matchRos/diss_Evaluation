...
class LissajousResponsePublisher:
    ...
    def config(self):
        self.a = rospy.get_param("~a", 0.16)
        self.b = rospy.get_param("~b", 0.16)
        self.c = rospy.get_param("~c", 0.20)
    ...
    def run(self):
        ...
        # get object pose
        object_pose = rospy.wait_for_message(self.object_pose_topic, PoseStamped)
        ...
        for t in range(0, int(30 / self.velocity)):
            ...
            pose_stamped.pose.position.z = self.c * math.sin(self.omega_z * t * self.velocity) + self.delta_z + object_pose.pose.position.z
