...
# first load all wrench controllers
...
class TurnOnAllWrenchControllers:
    ...
    def config(self):
        ...
        self.controller_name = rospy.get_param('~controller_name', 'force_torque_sensor_controller')
        self.old_controller_name = rospy.get_param('~old_controller_name', '')

    def __init__(self):
        self.config()
        self.turn_on_wrench_controllers()
...
if __name__ == '__main__':
    rospy.init_node('turn_on_all_wrench_controllers')
    TurnOnAllWrenchControllers()
    rospy.spin()